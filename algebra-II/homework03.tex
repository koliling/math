...
\it \textbf{Section 10.4 Q3:} Show that
  ...
  \begin{proof}
    ...
    $(r_0+r_1i)\otimes(r_2+r_3i)$. This map is also injective: if
    $x=\sum_{j=1}^n (a_j+b_ji)\otimes(c_j+d_ji) \in\ker(\Phi)$, then
    \begin{align*}
      &\;\Phi\left( \sum_{j=1}^n (a_j+b_ji)\otimes(c_j+d_ji)
      \in\ker(\Phi)\right) =0\\
      \Rightarrow&\;\Phi\left( \sum_{j=1}^n (a_j+b_ji)\otimes(c_j+d_ji)
      \in\ker(\Phi)\right) =0\\
    \end{align*}
    
    So $\mathbb{C}\otimes_\mathbb{R}\mathbb{C}
