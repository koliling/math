...
\it \textbf{Q4:} Let $0\rightarrow A\xrightarrow{f}B \xrightarrow{g}C
  \rightarrow 0$ be a short exact sequence of left $F$-modules and $F$ a
  right $R$-module.
  \begin{enumerate}[label={(\alph*)}]
    \item Show that if $R$ has an identity and $F$ is a free right
      $R$-module, then $0\rightarrow F\otimes_RA\xrightarrow{1\otimes
      f}F\otimes_RB \xrightarrow{1\otimes g}F\otimes_RC \rightarrow 0$ is
      exact.
      \begin{proof}
      \end{proof}

    \item Show that if $F$ is not free then the sequence in (a) is not
      necessarily exact.
      \begin{proof}
      \end{proof}
  \end{enumerate}
