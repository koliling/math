...
\it \textbf{DF 13.6.8:} Let $l$ be a prime and let $\Phi_l(x)
  ...
  modulo $p$ for any prime $p$. Let $\xi$ denote any fixed primitive $l$-th
  root of unity.
  ...
  \begin{enumerate}[label={(\alph*)}]
    ...
    \item Suppose $p\neq l$ and let $f$ denote the order of $p\mod l$, i.e.
      ...
      $\xi\in\mathbb{F}_{p^n}$. Conclude that the minimal polynomial of
      $\xi$ over $\mathbb{F}_p$ has degree $f$.

      \begin{proof}
        Let $n\in\mathbb{N}$ a positive integer such that
        $\xi\in\mathbb{F}_{p^n}$. Since $\mathbb{F}_{p^n}^\times$ is a
        cyclic group, it must be generated by some element $a$ with order
        (as a group under multiplication) $p^n-1$. Then $\xi=a^k$ for some
        $k<p^n$. Now $\xi^l=1$, so $a^{kl}=1$. But $a$ has order $p^n-1$,
        ...
        Therefore $\mathbb{F}_{p^n}^\times$ will contain $\xi$. \\

        Since $\mathbb{F}_{p^n}$ contains $\xi$ if and only if
        $p^n\equiv1\mod l$, the given $n=f$ is the smallest power $p^n$ of
        $p$ with $\xi\in\mathbb{F}_{p^n}$. \\

        Therefore, $\mathbb{F}_{p^f}=\mathbb{F}_p(\xi)$, so the degree of
        the minimal polynomial of $\xi$ over $\mathbb{F}_p$ is
        \[[\mathbb{F}_p(\xi):\mathbb{F}_p] =[\mathbb{F}_{p^f}:\mathbb{F}_p]
        =f.\]
      \end{proof}
  \end{enumerate}
\end{document}
