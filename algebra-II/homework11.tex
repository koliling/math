...
\it \textbf{DF 14.3.11:} Prove that $x^{p^n}-x+1$ is irreducible over
  ...
  \begin{proof}
    ...
    Galois group is always cyclic, there must be some
    $\sigma\in\text{Gal}(\mathbb{F}_p(\alpha)/\mathbb{F}_{p^n})$ that
    generates $\text{Gal}(\mathbb{F}_p(\alpha)/\mathbb{F}_{p^n})$. This
    $\sigma$ must map
    ...
    [\mathbb{F}_{p}(\alpha):\mathbb{F}_{p^n}]=np$. Yet $p^n$ rises much
    faster than $np$, so unless $n=1$ or
    $n=p=2$, we will not have $p^n=np$. \\
  \end{proof}
\end{document}
