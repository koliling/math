...
\begin{enumerate}[label={\bf Q\arabic*:}]
  ...
  \item \it Assume $\varphi(x,y)$ is $\Delta_1$ in $\mathbb{N}$ and
    ...
    \begin{proof}
      ...
      Comment: I don't think $\varphi$ needs to be $\prod_1$ for i to be
      computable as a function.
    \end{proof}
  ...
  \item \it A theory is complete if and only if for all sentences
    ...
    \begin{proof}
      ...
      recursive, the theory of $Q$ would be decidable, contradicting
      Church's Theorem (Corollary 68). Since the axioms of $Q$ is
      recursive, we can enumerate all possible proofs from the axioms, and
      thus also enumerate all theories of $Q$. So to decide if a given any
      sentence $\varphi$ is a theory of $Q$, we enumerate all the $Q$
      theories until we reach either $\varphi$ or $\neg\varphi$; we will
      always reach one or the other since $Q$ is complete by assumption.
      Thus we can decide whether $\varphi$ is a theory of $Q$ by checking
      if $\varphi$ is enumerated. Since the described procedure is
      algorithmic, it is recursive by the Church-Turing thesis, so the
      theory of $Q$ is decidable, a contradiction.
    \end{proof}
\end{enumerate}
\end{document}
