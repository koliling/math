...
\it \textbf{Soare 5.3.14:} A set $S$ is introreducible if $S\leq_T T$ for
  ...
  \begin{proof}
    ...
    smallest index $u\in\overline{D}$ such that $u>t$ and $a_u>a_t$. Such
    $u$ must exist since $\overline{D}$ is infinite and since the $a_s$ are
    distinct, therefore this question is recursive in $\overline{D}$.
    Since $u\in\overline{D}$, all elements $a_u,a_{u+1},\ldots$ must be
    greater than $a_{u-1}$, and therefore also greater than $a_{t-1}$. Thus
    $t\in\overline{D}$ if and only if the elements
    ...
    decide if $t$ lies in $\overline{D}$.
  \end{proof}
