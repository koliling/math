...
\it \textbf{Soare 6.1.4:} Modify the proof of Theorem 6.1.1 to build an
  ...
  \begin{proof}
    Fix an enumeration $\{(e_s,i_s)\}_{s\in\omega}$ of of $\omega^2$. We
    construct the sequence in stages $s$ so that for each $i\in\omega$,
    that $A_i=\cup_{s\in\omega} A_{i}^s$, where $A_i^0=\emptyset$,
    $A_i^s\in2^{<\omega}$, and $A_i^s\prec A_i^{s+1}$, and
    $\sigma_j^0=\emptyset$. At stage $s$, we extend a finite number of
    initial segments $A_i^s$'s to satisfy the restriction
    \begin{equation}
      R_{e_s,i_s}: A_{i_s} \neq \Phi_{e_s}^{\oplus_{j\neq i_s} A_j},
      \label{eq:join}
    \end{equation}
    and to preserve the initial segment of $\oplus_{j\neq i_s}A_j$ that is
    involved in the computation to satisfy the restriction. To determine
    which $A_i^s$ need to be extended in order to preserve the computation,
    we fix any enumeration $\{(x_r,j_r)\}_{r\in\omega}$ of $\omega^2$ so
    that we may consider $\oplus_j A_j$ as a function in $2^\omega$. \\

    At stage 0, initialize $A_i^0=\emptyset$ for all $i\in\omega$. \\

    At stage $s+1$, we want to satisfy the restriction $R_{e_s,i_s}$. Now
    there are only a finite number indices $k_1,\ldots,k_n$ that are not
    equal to $i_s$ and whose initial segment $A_{k_l}^s$ is non-empty. We
    ask the $\emptyset'$-question
    \begin{center}
      \textit{Is there an extension $\sigma \succ A_{k_1}^s\oplus \ldots
      \oplus A_{k_n}^s\in 2^{<\omega}$ that is consistent with each
      $A_{k_m}^s$ and with $A_{i_s}=\emptyset$ such that $\Phi_{e_s}^\sigma
      (|A_{i_s}^s|)$ converges?}
    \end{center}

    If no such $\sigma$ exists set $A_{i_s}^{s+1}=A_{i_s}^{s\frown}0$ (to
    ensure that $A_{i_s}$ is total eventually), and
    $A_{k_m}^{s+1}=A_{k_m}^{s}$ for each $m\in\{1,\ldots,n\}$. \\

    On the other hand, if $\sigma$ exists, we want to extend both
    $A_{i_s}^s$ and a finite number of $A_{j}^s\subseteq \sigma$ where
    $j\neq i_s$ so that we can preserve the computation of
    \[\Phi_{e_s}^\sigma (|A_{i_s}^s|) \downarrow \neq
    A_{i_s}(|A_{i_s}^s|).\]

    To preserve $A_{i_s}(|A_{i_s}^s|)$, set $A_{i_s}^{s+1}$ to be
    $A_{i_s}^s$ concatenated by the first value in $\{0,1\}$ that is
    different from $\Phi_{e_s}^\sigma (|A_{i_s}^s|)$. \\

    To preserve $\sigma$ used in the computation of $\Phi_{e_s}^\sigma
    (|A_{i_s}^s|)$, recall the enumeration $\{(x_r,j_r)\}_{r\in\omega}$ of
    $\omega^2$ that we have fixed. Let $l_1,\ldots,l_u$ be the indices
    different from $i_s$ such that $(x,l_v)$ appears in
    $(x_0,j_0),\ldots,(x_{|\sigma|-1},j_{|\sigma|-1})$ for some
    $x\in\omega$. Observe that $\{k_1,\ldots,k_n\}\subseteq
    \{l_1,\ldots,l_u\}$ by construction. For each such index $l_v$, let
    $x_v$ be the largest $x$-coordinate that appears with index $l_v$ in
    $(x_0,j_0),\ldots,(x_{|\sigma|-1},j_{|\sigma|-1})$; i.e. let \[x_v
    =\max \{x\in\omega: (x,l_v)\in
    \{(x_0,j_0),\ldots,(x_{|\sigma|-1},j_{|\sigma|-1})\}\}.\] Then set
    $A_{l_v}^{s+1}\in 2^{<\omega}$ to be the smallest (with respect to
    lexicographical order) string of length $\max(x_v+1,|A_{l_v}^s|)$ that
    is consistent with both $\sigma$ and with $A_{l_v}^s$; such string must
    exist since $\sigma$ was chosen to be consistent with $A_{l_v}^s$.
    ...
    %\[(\exists t) (\exists \sigma_1 \succ A_{k_1}^s) \ldots (\exists
    %\sigma_n \succ A_{k_n}^s) \Phi_{e_s,t}^{\oplus_{l=1}^n
    %\sigma_{l}}(|A_{i_s}^s|) \downarrow?\]
  ...
  \begin{enumerate}[label={(\roman*)}]
    \item Show that there is a u.c.e. sequence of c.e. sets
      ...
      \begin{proof}
        ...
        A_i^{s+1}$. Fix an enumeration of $\{(x_r,i_r)\}_{r\in\omega}$ of
        $\omega^2$. Fix also an enumeration $\{\langle
        e_r,i_r\rangle\}_{r\in\omega}$ of all $(e,i)$ pairs. For each
        $\langle e,i\rangle$, we want to satisfy the restriction
        \[R_{\langle e,i\rangle}: A_i\neq\Phi_e^{\oplus_{j\neq i}A_j},\]
        giving higher priority to requirements $R_{\langle e,i\rangle}$
        where $\langle e,i\rangle$ is enumerated earlier. \\
