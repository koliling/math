...
\it \textbf{Soare 6.1.5:} A partially ordered set $\mathcal{P}=(P,\leq_P)$
  ...
  \begin{proof}
    ...
    By a similar argument of considering multiples of periods, we can show
    ...
    the set of direct descendants of $s+1$ from the same set. If $A$ and
    $D$ are non-empty, then map $s+1$ to any element that lies strictly
    between $\varphi(a_0)\wedge\ldots\wedge\varphi(a_{m-1})$ and
    $\varphi(d_0)\vee\ldots\vee\varphi(d_{n-1})$. If only $A$ is empty,
    map $s+1$ to any element that lies strictly between 0 and
    $\varphi(d_0)\vee\ldots\vee\varphi(d_{n-1})$. If only $D$ is empty,
    map $s+1$ to any element that lies strictly between
    $\varphi(d_0)\vee\ldots\vee\varphi(d_{n-1})$ and 1. Finally, if both
    $A$ and $D$ are empty, then $s+1$ must be incomparable with all
    elements in $\{0,\ldots,s\}$, so map $s+1$ to an element that is
    incomparable with $\varphi(0),\ldots,\varphi(s)$. Such an element
    exists from the above argument.
  \end{proof}
