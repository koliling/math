...
\it \textbf{Soare 7.3.7:} (Cooper-Epstein-Lachlan) Construct a pair of c.e.
  ...
  \begin{proof}
    ...
    Next, we show by induction on $n\in\omega$ that all requirements
    $R_{n}$ are satisfied by the first clause and the second clause and
    get injured only finitely many times. For the base case, $R_0$ is
    ...
    \textit{Base case:} Next we verify by induction on $\langle
    ...
    We first show that $R_{\langle e,i,j\rangle}$ was never acted by the
    second clause. From construction, if $R_{\langle e,i,j\rangle}$ acts
    by the second clause at stage $s$, then $R_{\langle e,i,j\rangle}$ will
    forever be satisfied by the second clause because $R_{\langle
    e,i,j\rangle}$ has highest priority and the use function $v(e,i,j,s)$
    preserves the computations involved in satisfying the second clause.
    More precisely, when the second clause is acted on, we have
    \[1=W_{e,s}(y) =\Phi_{j,s}^{D_s\restriction (v(e,i,j,s)+1)}(y)\] for
    some $y\in\omega$ that is enumerated into both $A$ and $B$ at stage
    ...
    is not satisfied, then $R_{\langle e,i,j\rangle}$ must have never been
    acted by the second clause. \\

    It can only be that $R_{\langle e,i,j\rangle}$ was never acted by
    either clauses throughout the construction, or that the requirement was
    acted by the first clause exactly once and never acted by the second
    clause. Assume it is the first case. Now $R_{\langle e,i,j\rangle}$ has
    the highest priority, so it is never injured. Thus we must have always
    chosen the same potential witness $x\in w_{e,i,j}$ for $R_{\langle
    e,i,j\rangle}$ for all stages of the construction. Also, since the
    requirement was never acted by the first clause, $x$ was never
    enumerated into $A$ or $B$. Then since $\Phi_i^{W_e}(x)=D(x)=0$ by
    assumption, at some large enough stage $s'\in\omega$, we must have
    \[W_e\restriction (s'+1) =W_{e,s'}\restriction (s'+1)\;\; \text{and}\;\;
    \Phi_{i}^{W_{e} \restriction (s'+1)}(x) =\Phi_{i,s'}^{W_{e,s'}
    \restriction (s'+1)}(x)=0.\]
    ...
    Finally, assume that $R_{\langle e,i,j\rangle}$ was acted by the first
    clause exactly once but never acted by the second clause. Let $s$ be
    the stage at which the requirement was acted by the first clause. Then
    ...
    enumerated into $W_e\restriction (u+1)$, so the computation for
    inequality in right clause above is preserved, i.e.
    ...
    \textit{Inductive step:} Assume all higher priority requirements
    $\{R_n\}_{n <\langle e,i\rangle}$ have been satisfied but $R_{\langle
    ...
    stage $s$ where all requirments $R_0,\ldots,R_{\langle e,i,j\rangle}$
