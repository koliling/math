...
\begin{enumerate}
  \item \it A set $A$ is REA iff $A=W^B\oplus B$ for some c.e. set $B$.
    ...
    \begin{proof}
      Given $A$, we would be able to compute $W^B$ and $B$. From these
      two subsets of $\omega$, we can compute the modulus $g\leq_T A$ of
      $A$ as follows. Given arbitrary $x\in\omega$, find the smallest
      $s\in\omega$ such that $B_s\restriction s$ can compute $A\restriction
      x$. That is, let $s\geq \lceil x/2\rceil$ be the least integer such
      that
      \[B_s\restriction s =B\restriction s,\]
      and
      \[W^{B_s\restriction s}_{s}(y) =W^B\restriction(y)\]
      for all $y\leq \lfloor x/2\rfloor$. \\
      ...
      compute $A$, making $A$ computable. Therefore $g$ is hyperimmune. \\
