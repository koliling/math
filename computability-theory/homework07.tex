...
\begin{enumerate}
  ...
  \item \it Show there is a hyperimmune-free degree. Hint: you have to
    ...
    \begin{proof}
      ...
      then for all paths $f$ in $T_{e+1}$, $\Phi^f_e$ will be total.
      If $n$ exists, set $T_{e+1}=U_{e,n}$; then for all paths $f$ in
      $T_{e+1}$, $\Phi^f_e$ will not converge at input $n$. \\
      ...
      computable, and for every path $Y$ in $T_{e+1}$, $\Phi^Y_e$ is total.
      We recursively construct a function $h$ such that for each path $Y\in
      T_{e+1}$, $h$ computably dominates the set whose characteristic
      function is $\Phi^Y_e$.  Then in particular $h$ will dominate the set
      whose characteristic function is $\Phi^X_e$. \\
