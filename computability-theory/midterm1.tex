...
\begin{enumerate}[label={\bf Q\arabic*:}]
  \item \it A tree $T$ is a set of finite strings of 0 and 1 and closed
    ...
    \begin{proof}
      ...
      n)^\frown0^\frown 2^k$, or $T_{(\tau\restriction n)^\frown1}$ is
      contained in $(\tau\restriction n)^\frown1^\frown 2^k$, and exactly
      one of these two conditions holds. Note that deciding if
      $T_{(\tau\restriction n)^\frown i}$ is contained in $\tau\restriction
      n^\frown i^\frown 2^k$ is computable because $T$ is computable. Thus
      if the former is true, then $\tau\not\in p$, otherwise $\tau\in
      p$. \\
  ...
  \item \it Use Problem 3 and Rosser's Theorem to show there is a
    ...
    \begin{proof}
      Rosser's theorem says that given any consistent
      $T\supseteq\text{PA}$ and sentence $\varphi$, there exists a sentence
      $\psi(\varphi)$ that is independent of $T$. More precisely, this
