...
\begin{enumerate}[label={\bf Q\arabic*:}]
  ...
  \item \it Use Problem 3 and Rosser's Theorem to show there is a
    ...
    \begin{proof}
      ...
      say:
      \begin{center}
        ``\textit{For any proof of me from any consistent theories in
      ...
      these consistent theories proves $\varphi$; we can assume without
      loss of generality that this theory is $T_1$. Then there is a
      ``real'' proof coded as some natural number $n_1\in\omega$ such that
      $T_1\vdash\varphi$ via this proof. By choosing the shortest proof of
      $\varphi$ from $T_1$, we can assume that no proof shorter than $n_1$
      can give $T_1\vdash\varphi$. So we have
      ...
      $\varphi$ of length $n_3$ from one of the consistent theories
      $T_1,\ldots,T_n$, such that the proof is shorter than all proofs (if
      any) of $\neg\varphi$ from any consistent theory in $T_1,\ldots,T_n$.
      Now we already have $T_2\vdash\neg\varphi$ via a proof of length
      $n_2$, so from the assumption of $\neg\varphi$, we have $n_3<n_2$,
      and in particular, implies that this proof is a ``real'' proof, and
      ...
      also cannot be $T_2$ from consistency of $T_2$. Assume it is $T_3$
      that proves $\varphi$, and let $n_3$ be the length of shortest proof
      of $\varphi$ from $T_3$. So we have
      \[\exists\; \text{shortest proof of}\; T_3\vdash\varphi\;
      \text{with length}\; n_3<n_2<n_1\in\omega.\]

      Continueing in this manner, we can 
