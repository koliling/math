...
\begin{enumerate}[label={\bf Q\arabic*:}]
  ...
  \item \it Fix the above tree $T$. Show for every infinite computable
    ...
    \begin{proof}
      Let $H_{\tilde{T}}$ be the theory of PA plus $\Phi_{\tilde{T}}$, the
      sentences that all segments that $\tilde{T}$ agree with, plus
      $\Omega_{\tilde{T}}$, the negation of the sentences that are
      associated with segments not in $\tilde{T}$. \\

      More precisely, let
      \[H_{\tilde{T}} :=\text{PA} \cup\Phi_{\tilde{T}}
      \cup\Omega_{\tilde{T}},\]
      where
      \[\Phi_{\tilde{T}} :=\{\varphi_i:i\in\omega\; \wedge\; (\sigma(i)=1\;
      \forall \sigma\in \tilde{T}_{i})\},\]
      and
      \[\Omega_{\tilde{T}} :=\{\neg(\theta_{\sigma,0} \wedge\ldots
        \wedge\theta_{\sigma,|\sigma|-1}): \sigma\in
        2^{<\omega}-\tilde{T}\},\]
      where
      \begin{equation*}
        \theta_{\sigma,i} :=
      ...
      Note that $\Phi_{\tilde{T}}$ and $\Omega_{\tilde{T}}$ are computable
      because $\tilde{T}$ is, thus $H_{\tilde{T}}$ is also computable. \\

      Clearly, all paths in $\tilde{T}$ will satisfy $H_{\tilde{T}}$ by
      definition of $H_{\tilde{T}}$. To prove the reverse inclusion, let
      $p$ be a path in $T$ but not in $\tilde{T}$. Then there must be some
      initial segment $\sigma\prec p$ that is not in $\tilde{T}$. Then the
      theorem associated with $p$ will satisfy the sentence
      \[\theta_{\sigma,0} \wedge\ldots \wedge\theta_{\sigma,|\sigma|-1},\]
      and therefore cannot satisfy the negation of the sentence which is
      contained in $H_{\tilde{T}}$.
    \end{proof}
\end{enumerate}
\end{document}
