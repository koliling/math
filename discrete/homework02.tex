...
\begin{enumerate}[label={\bf Q\arabic*:}]
  ...
  \item \it \textbf{Section 13 Problem 2:} The Bonferroni inequalities say
    ...
    \begin{proof}
      ...
      \end{align*}
      Thus it suffices to show that the number of subsets of $J$ of odd
      size exceeds the number of subsets of $J$ of even size if and only if
      $k$ is odd. \\

      Recall that there is a bijection
      $\pi:\mathcal{P}([n])\rightarrow\mathcal{P}([n])$, defined by
      \[\pi(S):=
        \begin{cases}
        ...
        \end{cases},\]
      which sends subsets
      of $[n]$ of odd sizes to subsets of $[n]$ of even sizes and
      vice-versa.
    \end{proof}
