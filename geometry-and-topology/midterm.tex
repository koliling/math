  ...
  \item
    \begin{enumerate}
      \item Let $x_1,\ldots,x_k\in S^2$ be distinct points and let $p\in
        ...
        \begin{proof}
          When $k=1$, space $S^2\setminus\{x_1\}$ is a sphere with a
          punctured hole, which is homeomorphic to $\mathbb{R}^2$, which as
          trivial fundamental group $1=F_0$. When we remove an additional
          $(k-1)$ points from $\mathbb{R}^2$, we get a space homeomorphic
          to $\mathbb{R}^2\setminus\{x_2,\ldots,x_k\}$, which is a strong
          deformation retract to the wedge of $k$ circles at a fixed point
          $p$. Such a wedge has fundamental group $F_{k-1}$. Summarizing,
          \begin{align*}
