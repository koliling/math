    ...
    \begin{enumerate}[label={\bf(\arabic*)}]
      ...
      \item Let $\aleph_1$ be the first uncountable cardinal. Show that
        ...
        \begin{proof}
          ...
          other than the smallest one has a direct predecessor''. This
          sentence is satisfied by $\omega$ but not by $\aleph_1$, because
          $\aleph_0\in\aleph_1$ does not have a direct predecessor. We can
          write $\varphi$ in first-order language as
          \[\varphi:= \forall x\; (\exists y\; y<x)\rightarrow\;
          \left[\exists z\; (z<x)\wedge \forall w\; (w<x\rightarrow(w<z\vee
          w=z)) \right].\]
        \end{proof}
