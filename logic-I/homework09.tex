...
\begin{enumerate}[label={\bf Q\arabic*:}]
  ...
  \item Let $T$ be the theory of vector spaces over $\mathbb{Q}$,
    ...
    \begin{proof}
      ...
      %First note that if $\mathcal{M}$ is finite, then since there can only
      %be a maximum of $|M|$ types in $S^{\mathcal{M}}_1(A)$ (one type for
      %each element in $M$), the assertion will be trivially true. Hence
      %we can assume that $\mathcal{M}$ is infinite. \\
