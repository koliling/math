...
\begin{frame}{Strengthening Lemma~\ref{lemma:fixed-machine}}
  To iterate Lemma~\ref{lemma:fixed-machine} successfully, we strengthen it
  by two conditions:
  ...
  \begin{lemma-strengthened*}[Strengthened Lemma \ref{lemma:fixed-machine}]
    ...
    ($\neq\emptyset$), and\\
    \textbf{Condition 2:} $X\restriction s\succ Z\restriction s$.
  \end{lemma-strengthened*}

  \vspace{1em}
  Assuming Lemma~\ref{lemma:fixed-machine} has been strengthened by
  condition 1, we can satisfy condition 2 by iterating the
  partially-strengthened lemma finitely many times.
\end{frame}
