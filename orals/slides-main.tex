...
machine)}
  \begin{lemma}
    ...
    that every subset of $X$ computes only paths outside $\mathcal{A}$ (via
    $\Gamma$).
  ...
  \vspace{1em}
  Observe that by finite use principle, given $\sigma$, the class
  $\mathcal{P}_{\sigma}$ of positive solutions $X\in[Z]^\omega$ whose
  subsets all compute $\sigma$ is $\Sigma_1^{1,Z}$:
  \begin{align*}
...
\begin{frame}{Subsets compute no path in $\mathcal{A}$ (fixed
machine)}
  \textbf{Case 1:} For some $n$, $\mathcal{P}_\sigma=\emptyset$ for every
  $\sigma\in\omega^n\cap\mathcal{A}$. Since $\mathcal{A}$ is compact, it is
  finitely branching. Let $\sigma_0,\ldots,\sigma_m$ be the nodes in
  $\mathcal{A}$ at level $n$. Iterate GP across $\sigma_i$ to get
