...
machines)}
  ...
  $\Gamma_0,\Gamma_1,\ldots$. Then, apply Lemma~\ref{lemma:fixed-machine}
  ...
  \vspace{1em}
  However, if we iterate Lemma~\ref{lemma:fixed-machine} naively, we may
  not end up with an infinite $X$. To ensure infiniteness, at stage $s$ of
  the iteration, we choose the $X_{s+1}\in[X_s]^\omega$ where $X_{s+1}$
  preserves the same first $s$-elements as $X_s$. We strengthen
  Lemma~\ref{lemma:fixed-machine} to ensure that such $X_{s+1}$ exists.
\end{frame}
