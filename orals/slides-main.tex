...
machines)}
  ...
  Lemma~\ref{lemma:fixed-machine}, we may end up with an $X$ that is
  finite. To ensure infiniteness, at stage $s$ of the iteration, we choose
  the $X_{s+1}\in[X_s]^\omega$ where $X_{s+1}$ preserves the same first
  $s$-elements as $X_s$. We strengthen Lemma~\ref{lemma:fixed-machine} to
