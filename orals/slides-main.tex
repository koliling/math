...
machine)}
  ...
  \vspace{0.5em}
  \textbf{Case 0:} For every $n$, GP has positive solutions for some
  $\sigma_n\in\omega^n\cap\mathcal{A}$. Observe that by finite use, given
  $\sigma$, the class $\mathcal{P}_{\sigma}$ of positive solutions
  $X\in[Z]^\omega$ is $\Sigma_1^{1,Z}$:
  \begin{align*}
    ...
    =&\{X\in[Z]^\omega: \text{All finite subsets of}\; X\;
      ...
      \in\Sigma_1^{1,Z}.
  \end{align*}
  Then by compactness of $\mathcal{A}$, the $\Sigma_1^{1,Z}$-set
  \[\{f:(\forall \sigma\prec f)\; \mathcal{P}_\sigma \neq \emptyset\}\]
  is non-empty and contained in $\mathcal{A}$, $\Rightarrow\Leftarrow$.
\end{frame}
