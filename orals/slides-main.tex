...
\begin{frame}{Strengthening Lemma~\ref{lemma:fixed-machine}}
  ...
  \vspace{1em}
  Assuming Lemma~\ref{lemma:fixed-machine} has been strengthened only by
  the first condition, we can immediately satisfy the second one as
  follows: Apply the partially-strengthened lemma using the Turing
  functional that adds $Z\restriction s$ to the input before sending it to
  $\Gamma$.
\end{frame}

\begin{frame}{$X\subseteq Z$ that preserves initial segment of $Z$}
