...
\begin{frame}{$\Sigma_1^1$-immunity basis theorem}
  We want $X$ whose trees avoid $\mathcal{A}$ regardless of the machine
  used to compute the tree. The previous claim gives us enough $X$'s that
  work for a fixed machine to guarantee the existence one that works for
  all machines.
...
\begin{frame}{$\Sigma_1^{1,Z}$-immunity basis theorem}
  ...
  \begin{thm}[$\Sigma_1^{1,Z}$-immunity basis]
    $\mathcal{A}$ compact, every $\Sigma_1^{1,Z}$-subset contains a path
  ...
  sense: either all paths computed by these subsets lie outside
  $\mathcal{A}$, or all trees computed share arbitrarily long branches
  that lie outside $\mathcal{A}$.
\end{frame}

\begin{frame}{Subsets compute paths outside $\mathcal{A}$, Or same node
outside $\mathcal{A}$}
  \begin{lemma}
    $\mathcal{A}$ compact, every $\Sigma_1^{1,Z}$-subset contains a path
    (if any) outside $\mathcal{A}$. Fix Turing $\Gamma:2^{<\omega}
    \rightarrow \omega^{<\omega}$. Then there exists $X\in[Z]^\omega$ such
    that one of these cases hold:

    \textbf{Case 1:} Every subset of $X$ computes only paths outside
    $\mathcal{A}$ (via $\Gamma$)

    \textbf{Case 2:} For every $n\in\omega$, some $\sigma\in\omega^n$ with
