...
\begin{frame}{WWKL $\nleq_{\text{soc}}$ RT}
  \begin{itemize}
    ...
    \item Now given arbitrary infinite set $Z$, by relativizing $c$ with
      respect to $Z$ will give us a $c$-homogeneous set $X\in[Z]^\omega$.
      By assumption, every element in $[X]^\omega$ computes a path in $T$.
    \item Thus every infinite set $Z$ has an infinite subset $X$ such that
      every element in $[X]^\omega$ computes a path in $T$.
