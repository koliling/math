...
\begin{frame}{Strengthening Lemma~\ref{lemma:fixed-machine}}
  ...
  \begin{lemma-strengthened1*}[$\Sigma_1^{1,X}$-sets compute no path in
  $\mathcal{A}$]
    $\mathcal{A}$ compact, contains no $\Sigma_1^{1,Z}$-set
  ...
  \begin{lemma-strengthened2*}[$X$ preserves given initial-segment of $Z$]
...
preserves $Z\restriction s$}
  ...
  d_i$, the same is ensured via $\Gamma$.
...
\begin{frame}{Subsets compute no path in $\mathcal{A}$ (all machines)}
  ...
  computes no path in $\mathcal{A}$ via $\Gamma_i$.
\end{frame}
