...
\begin{frame}{Subsets all compute $\sigma$ or all avoid $\sigma$}
  Fix Turing machine $\Gamma$, node $\sigma$.
  \begin{fact}[Galvin-Prikry]
    There exists an infinite subset $X$ such that one of these holds:
    \textbf{Case 1:} All subsets of $X$ compute $\sigma$ (via
    $\Gamma$)\\
    \textbf{Case 2:} All subsets of $X$ do not compute $\sigma$ (via
    $\Gamma$)
  \end{fact}

  \textbf{Proof idea:} The class of infinite sets that compute $\sigma$
  forms an open set in $2^\omega$. Being open provides enough ``structure''
  for homogeneity in some areas.

  \begin{coro}[Relativized Galvin-Prikry]
    ...
    $\Gamma$)
  \end{coro}
  Code each $X\in[Z]^\omega$ as a set in $[\omega]^\omega$.
\end{frame}
