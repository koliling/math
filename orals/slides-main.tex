...
\begin{frame}{Hard direction of main theorem}
  ...
  \begin{claim}[$\Sigma_1^1$-subset compute paths outside $\mathcal{A}$
  via fixed machine]
...
\begin{frame}{$\Sigma_1^1$-immunity basis theorem}
  ...
  \textbf{Idea:} The set of $X$'s from each predicate is so dense
  (from previous claim) that they contain a common $X$.
...
\begin{frame}{$\Sigma_1^{1,Z}$-immunity basis theorem}
  ...
  \vspace{2em}
  We can make a similar conclusion even after accounting for paths computed
  by subsets if we fix the Turing machine and allow ourselves to choose any
  infinite $X$.
\end{frame}
