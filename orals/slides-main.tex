...
\begin{frame}{Subsets compute trees avoiding $\mathcal{A}$}
...
\begin{frame}{Subsets compute trees avoiding $\mathcal{A}$}
  \textbf{Proof idea of main theorem:} Want $X$ whose subsets compute trees
  avoiding $\mathcal{A}$. By Galvin-Prikry, given any node in
  $\mathcal{A}$, we can always find $X$ whose subsets compute trees that
  either all contain that node, or all avoid that node. The former cannot
  be true for every initial segment of a path in $\mathcal{A}$, otherwise
  by compactness of $\mathcal{A}$, the path will induce a $\Sigma_1^1$-set
  ...
  \end{itemize}
  the assertion follows from Galvin-Prikry.
