...
\begin{frame}{Subsets compute paths outside $\mathcal{A}$}
  ...
  \begin{Fact}[Galvin-Prikry]
    ...
    one of these holds:\\
    \begin{itemize}
      \item All subsets of $X$ compute $\sigma$ (via $\Gamma$)
      \item All subsets of $X$ do not compute $\sigma$ (via $\Gamma$)
    \end{itemize}
  \end{Fact}
...
\begin{frame}{Subsets compute paths outside $\mathcal{A}$}
  ...
  $\mathcal{A}$. Fix machine. Then given any node, GP finds an $X$ whose
  subsets compute trees that either all contain that node ($X$ is positive
  solution), or all avoid it ($X$ is negative solution). Exhaust cases:
  ...
  get $X$ where all these nodes are avoided. Such $X$ will work. Note there
  are only finitely many nodes to avoid since $\mathcal{A}$ is compact.

  %\vspace{0.5em}
  %\textbf{Case 1:} GP answers positively for some node outside $X$.

  \vspace{1em}
  Thus GP gives us $X$ that works for a fixed machine. To find an $X$ that
  works across all machines, iterate across them, constructing
  decreasing subsets of $X$, then take intersection.
\end{frame}

\begin{frame}{Subsets compute paths outside $\mathcal{A}$ (fixed
$\Gamma$)}
  \begin{lemma}
    $\mathcal{A}$ compact, contains no $\Sigma_1^{1,Z}$-set
    ($\neq\emptyset$). Fix Turing $\Gamma:2^{<\omega}
    \rightarrow \omega^{<\omega}$. Then one of these hold:

    \vspace{0.5em}
    \textbf{Case 1:} There is $X\in[Z]^\omega$ such that every subset of
    $X$ computes only paths outside $\mathcal{A}$ (via $\Gamma$).
...
\begin{frame}{Subsets compute paths outside $\mathcal{A}$ (fixed
$\Gamma$)}
  Observe that the class $\mathcal{Q}_{\sigma}$ of $X\in[Z]^\omega$ whose
  subsets all compute a given $\sigma$ is $\Sigma_1^{1,Z}$:
  \begin{align*}
    \mathcal{Q}_{\sigma}:= &\{X\in[Z]^\omega: \text{All subsets of}\; X\;
      \text{compute}\; \sigma\; (\text{via}\; \Gamma)\}\\
    =&\{X\in[Z]^\omega: \text{All finite subsets of}\; X\;
      \text{compute}\; \sigma\; (\text{via}\; \Gamma)\}.\\
  \end{align*}

  \textbf{Case 2:} For some $n$, $\mathcal{Q}_\sigma=\emptyset$ for every
