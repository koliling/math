...
\begin{frame}{$X\subseteq Z$ where $\Sigma_1^{1,X}$-sets compute no path in
$\mathcal{A}$}
  \begin{lemma-strengthened1*}
    $\mathcal{A}$ compact, contains no $\Sigma_1^{1,Z}$-set
    ($\neq\emptyset$). Fix Turing functional $\Gamma$. Then
  ...
  For $\mathcal{A}$ to contains no $\Sigma_1^{1,X}$-set ($\neq\emptyset$),
  we choose $X$ from the original Lemma~\ref{lemma:fixed-machine} with the
  desired property. Recall in the proof of Lemma~\ref{lemma:fixed-machine},
  the $X$ found lies in $\mathcal{N}_{\sigma_0}\cap
  \ldots\cap\mathcal{N}_{\sigma_m}$,
  where $\mathcal{N}_{\sigma_i}\in\Sigma_1^{1,X_i}$ and $X_0=Z$.
\end{frame}
