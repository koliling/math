...
($\leq_{\text{soc}}$)}
  We formalize the notion of strength between problems.
  ...
  \begin{define*}[Strongly Omnisciently Computably Reducible]
    Problem \textbf{P} is \textbf{soc}-reducible to problem \textbf{Q} if
    for every \textbf{P}-instance \textit{I}, there is a
    \textbf{Q}-instance \textit{J} such that every solution to \textit{J}
    computes a solution to \textit{I}.
  \end{define*}
\end{frame}
