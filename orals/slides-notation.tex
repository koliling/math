...
($\leq_{\text{soc}}$)}
  ...
  of positive measure has infinite nodes, so WKL asserting the existance of
  a path for binary trees with infinite nodes implies the existance of a
  path for binary trees with positive measure.

  \vspace{0.5em}
  A more interesting comparison with less straightforward answer is
  \newtheorem*{question*}{Question}
  \begin{question*}
    Which notion is stronger, homogeneity or randomness?
  \end{question*}
  Given a path that is homogeneous under some coloring, does that say
  anything about how random it is? Likewise, if a given path is random, can
  we use it to find homogeneous paths in some coloring?
