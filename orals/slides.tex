...
element in}\; [A]^\omega\}) =0$}
  ...
  for when new elements of $A$ have appeared. This gives a recursive
  function dominating $p_A$.\\
  \vspace{1em}

  Fix Turing functional $\Phi$. Write $\mathcal{C} :=\{X:
  \Phi^X\in[A]^\omega\}$. Since the measure of a countable union of sets of
  measure 0 is 0, and there are only countably many Turing functionals, it
  suffices to show $\mu(\mathcal{C})=0$. Assume $\mu(\mathcal{C})=4m>0$.
\end{frame}
