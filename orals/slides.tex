...
\begin{frame}{Bi-Hyperimmune sets $A$ exist}
  \begin{itemize}
    ...
    \item Fix $\emptyset''$-enumeration $f_0,f_1,\ldots$ of all recursive
      functions.
    \item At stage $n=0$, apply diagonal argument and choose
      $p_A(0)=f_0(0)+1$ so that $p_A\not<f_0$. 
    \item Then the first $p_A(0)$ elements of $\bar{A}$ are
      $\{0,\ldots,p_A(0)-1\}$. To ensure $p_{\bar{A}}\not<f_0$, set the
      next element of $\bar{A}$ to exceed that of $f_0$, i.e. put all
      \[p_A(0),\ldots,f_0(p_A(0)+1)\]
      into $A$, and $f_0(p_A(0)+1)+1$ into $\bar{A}$, so that
      \[p_{\bar{A}}(p_A(0)+1) =f_0(p_A(0)+1)+1 >f_0(p_A(0)+1).\]
    \item Repeat for stages $n>0$ so that $p_A(2n)$ witnesses
      $p_A\not<f_{n}$ and $p_A(2n+1)$ witnesses $p_{\bar{A}}\not<f_{n}$.
  \end{itemize}
\end{frame}
