...
\begin{frame}{Contains non-empty $\Sigma_1^1$ subset $\rightarrow$
$\Pi_1^0$-encodable}
...
\begin{frame}{$\Pi_1^0$-encodable $\rightarrow$ Contains non-empty
$\Sigma_1^1$ subset}
  \begin{itemize}
...
\begin{frame}{Desired $G_{s+1}\in[G_s]^\omega$ exists}
  \begin{itemize}
    ...
    \item Want $G_1\in[G_0]^\omega$ such that $G_0\restriction
      n=G_0\restriction n$, and no $X\in[G_1]^\omega$ computes a non-empty
      subset of $\mathcal{C}$ via $\Gamma$.

    \item If we do not require $G_0\restriction n=G_0\restriction n$, then
      we can find $G_0$ with stronger property:
  \end{itemize}
