...
\subsection{Infinite Injury Constructions}
  Construct r.e. $A$ in stages satisfying negative requirements $N_e$ and
  positive requirements $P_e$.
  \begin{itemize}
    \item $N_e$: Restraint $r(e,s)$ prevent elements from being enumerated
      into $A$ to protect some computation. E.g.
      \[N_e: \Phi_e^A\neq \Phi_e^B\]
      for some fixed r.e. B.
  ...
  In particular, if there are infinite elements that can be enumerated into
  $A$ to satisfy $P_e$, suffices for
  \[\liminf\]
