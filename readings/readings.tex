...
\subsection{Tree Method for Infinite Injury Construction}
  ...
  Level $2e$ represents $N_e$ and level $2e+1$ represents $P_e$. Choose
  $\Lambda=\omega$ and
  \[T:= \{\alpha\in\omega^{<\omega}: (\forall e)[\alpha(2e+1)
  \in\{0,1\}]\}.\]

  At stage $s$, observe outcome of the first $2s$ requirements, and
  represent the outcomes by $\delta_s\in T$ where
  $|\delta_s|=2s$:
  \begin{align*}
    \delta_s(2e) &:=\text{guess for}\; \liminf_s r(e,s),\\
    \delta_s(2e+1) &:=
      \begin{cases}
        0, &\text{if we think}\; |C^{[e]}|=\omega,\\
        1, &\text{otherwise}\\
      \end{cases}\\
  \end{align*}
  ...
  Thus outcomes usually assigned such that if $a<_\Lambda b$ and we
  observe outcome $b$ infinitely often, then outcome $a$ cannot be true. \\

  To construct the r.e. sets, at stage $s$, observe the outcome
  $\delta_s\in T$, and play the $\delta_s$-strategy. At each stage $s$,
  ...
  left of $\alpha$: Set 
  \[r(\alpha,s) \geq \max\{r(\beta,s): \beta<\alpha\},\]
  \[r(\beta,s+1)=0\; \text{if}\; \beta<\alpha.\]
