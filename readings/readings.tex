...
\subsection{Promptly Simple Sets}
  ...
  \begin{proof}
    ...
    $N_e$. We construct r.e. $A$ such that for each $e\in\omega$, $A$
    ...
    requirements will not need to be satisfied.}\\

    \underline{Outline}\\
    At stage $s+1$, for each $e\leq s$, we find the first $i\leq s$ such
    that $P_{e,i}$ is not yet satisfied, and for which there is some 
  \end{proof}
\end{document}
