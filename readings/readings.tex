...
\subsection{Degrees of Maximal Sets}
  ...
  \begin{proof}
    ...
    \[c_f(x) =(\mu s) [\Phi_s^{A_s}(x) =\Phi^A(x)].\]
  ...
  \end{lemma}
  \begin{proof}
    Assume the lemma for $i<e$ and choose $n\geq e$ such that for all
    $m>n$, $\sigma(e-1,a_m)=\sigma(a_n)=\sigma_0$, and stage $s_0$ such
    that all $a_0,\ldots,a_n$ have stabilized. Assume by contradiction that
    $R_e$ is not satisfied. We define a recursive $h(x)$ not dominated by
    $c_{\overline{D}}(x)$ as follows.
  \end{proof}
\end{document}
