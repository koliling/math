...
\section{Arithmetical Hierarchy}
  ...
  \subsection{Lattice $\mathcal{D}$}
    ...
    \end{theorem}
    \begin{proof}
      The proof is non-constructive. From the paragraph after Soare's
      D5.6.1, the hyperimmune-free degrees are exactly those that are
      incomparable with $\emptyset'$. Also recall that a set is hyperimmune
      if and only if it is not computably dominated. The construction of
      Soare T9.5.1 gives a noncomputable set with a hyperimmune-free
      degree.
    \end{proof}
